...
void nnai_interpolate_flex(nnai* nn, const double* zin, double* zout, const unsigned int zinWidth, unsigned int dimension)
{
    int i;
    ...
    for (i = 0; i < nn->n; ++i) {
        ...
        for (idim=0; idim<dimension; idim++) {
           ...
           for (j = 0; j < w->nvertices; ++j) {
              ...
              z += weight * zin[w->vertices[j]*zinWidth+idim];
           }

           zout[i*dimension+idim] = z;
        }
    }
}
