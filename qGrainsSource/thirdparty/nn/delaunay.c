...
static void tio2delaunay(struct triangulateio* tio_out, delaunay* d)
{
    ...
    if (tio_out->segmentlist != NULL) {
       int i;
       if (nn_verbose)
          fprintf(stdout, "segmentlist:\n");
       for (i=0; i<tio_out->numberofsegments; i++) {
          int offset = i * 2;
          d->numberofsegments = tio_out->numberofsegments;
          d->segmentlist = malloc(d->numberofsegments * 2 * sizeof(int));
          memcpy(d->segmentlist, tio_out->segmentlist, d->numberofsegments *
                2 * sizeof(int));
          if (nn_verbose)
             fprintf(stdout, "  %d: (%d,%d)\n", i, d->segmentlist[offset],
                   d->segmentlist[offset+1]);
       }
    }
}
...
int numberofsegments (delaunay* d) { return d->numberofsegments; }
int *segmentlist (delaunay* d) { return d->segmentlist; }
